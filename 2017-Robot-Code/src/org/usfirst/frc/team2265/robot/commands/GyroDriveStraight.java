...
public class GyroDriveStraight extends Command {
	...
	//public GyroPIDDrive gpidDrive = new GyroPIDDrive();
	...
	protected void initialize() {
		GyroPIDDrive.gyro.reset();
		GyroPIDDrive.pid.setSetpoint(0);
		GyroPIDDrive.pid.setContinuous(true);
		GyroPIDDrive.pid.enable();
	...
	Drivetrain.rearRight.set(rightVel);
			...
			/*
			GyroPIDDrive.pid.calculate();
			gpidDrive.usePIDOutput(GyroPIDDrive.m_pidOutput);
			*/
	
			
	    lastError = error;
	    error = GyroPIDDrive.pid.getError();
	    derivative = (error - lastError)/ period;
	    errorSum += error*period;
	    output = error*GyroPIDDrive.Kp + derivative*GyroPIDDrive.Kd + errorSum*GyroPIDDrive.Ki;
	    Robot.gpidDrive.usePIDOutput(output);
